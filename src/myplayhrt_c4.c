/*
myplayhrt_c4.c                Copyright frankl 2025
...
d nanoseconds we use intervals of d*3/125 ticks of the counter.
For precision we use doubles for the time differences in ns and in ticks
and round to integers when needed.
...
#include <sys/prctl.h>
#include "cprefresh.h"
//#include "counttotick_aa64.h"

//inline void counttotick_aa64(long c);
...
/* some code is now in comments after we found a good value for 'shift' on
   C4, search for 'sstat' */
...
//printf(" nsec = %lf   ntick = %lf \n", nsec, ntick);
    ...
    if (verbose) {
        fprintf(stderr, "playhrt: Step size is %lf nsec.\n", nsec);
    }
  ...
  //printticks(liticks, tcount);
          ...
          while (sz < flen) {
             ...
             /* The isb mrs command  takes 23.3608 ns while the tick counter
                ...
                the last mrs checks if the tick was updated wrt x7
                if yes the last mrs was > 11.6 ns after the tick update
                otherwise before, then we do another 10 add x8 lines
                This way we end the __asm__ within a specified range of ~12 ns.

                It is advantageous to adjust parameters such that the time between
                commits is an integer multiple of the tick length 125/3 ns. */
             __asm__ __volatile__ (
                    "mov x8, #0 \n\t"
             ".LOOP: \n\t"
                    "isb  \n\t"
             ...
             : [et] "r" (etick)
             : "x7", "x8" ); 
             /* This simpler loop always stops in interval 
               of 0..23.36 ns after the tick. */
             /*__asm__ __volatile__ (
             ".LOOP: \n\t"
                    "isb  \n\t"
                    "mrs x7, cntvct_el0 \n\t"
